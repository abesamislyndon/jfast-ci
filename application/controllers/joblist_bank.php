 ...
 function __construct()
 {
   ...
   public function job_approval(){
     ...
     if($this->session->userdata('logged_in')&&$this->session->userdata['logged_in']['role_code'] == '1')
     {
        ...
        $this->pagination->initialize($config);
        $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $data['job_list_incoming'] = $this->job_delivery_model->show_job_approval($config["per_page"], $page);
        $data["links"] = $this->pagination->create_links();
  ...
  public function job_invoice_list(){
    ...
    public function individual_search_regular(){
        ...
        $id = $this->uri->segment(3);
        $data['individual'] = $this->job_delivery_model->show_individual($id);
        $data['individual_item_type'] = $this->job_delivery_model->show_individual_item_type1($id);
